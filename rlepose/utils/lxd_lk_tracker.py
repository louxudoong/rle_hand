...
def filter_background_motion(last_motion, motionxy, threshold_angle):
    # 计算每个角点与上一时刻运动向量的夹角
    cos_sim = np.dot(motionxy, last_motion) / (np.linalg.norm(motionxy, axis=1) * np.linalg.norm(last_motion))
    angles = np.arccos(cos_sim)
    
    # 根据夹角判断是否为背景角点的运动
    background_mask = angles > threshold_angle
    
    # 剔除背景角点的运动数据
    filtered_motionxy = motionxy[~background_mask]
    
    return filtered_motionxy
...
def LK_Tracker(frame, count, last_tracks, prev_gray = None, track_len = 10, detect_interval = 5):
    ...
    '''
    返回的参数列表依次为：
    vis: 显示当前帧预测点和历史轨迹的图像;
    prev_gray: 当前帧的gray图像，将作为下一时刻的输入
    last_tracks: 更新了当前帧预测点的track信息，将作为下一时刻的输入
    '''
    return vis, prev_gray, last_tracks, motion, p1, p0


def LK_Tracker_rle(frame, pts, count, last_tracks, prev_gray = None, track_len = 10, detect_interval = 5):
    # 使用rle输出的关键点坐标作为track pts，shape统一为（21 * 2）
